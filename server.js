...
async function ensureTables() {
  ...
  // Persistent chat history for actual context, not just what the client sends
  await pool.query(`
  ...
  // Store Stripe events to avoid duplicate processing
  await pool.query(`
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (exists.rows.length === 0) {
        ...
        // We have seen this event, so we can return early
    ...
    try {
      switch (event.type) {
        case "checkout.session.completed": {
          ...
          // Ensure state exists and set default banned words
...
// ===== GLOBAL VOICE RULES =====
// This is Becky mode: sharp, certain, alive, UK English, no em dash, no beige, no corporate drift.
const GLOBAL_RULES = `
...
// Translate stored preferences into hard system rules
...
// ===== STATE API (for UI, not chat magic words) =====
...
// ===== USER INFO =====
// Protect this. Do not leak api_token without token match.
...
app.get("/chat-ui/:email", (req, res) => {
  // In a real setting you might want to check a short-lived token here.
  res.sendFile(path.join(__dirname, "public", "chat.html"));
});
...
app.post("/chat", async (req, res) => {
  ...
  try {
    ...
    // Store user message in history right away
    ...
    // Onboarding gating. If any of avatar, my_profile, preferences exist the model must not start onboarding
    ...
    // Fetch history from DB, not from client
